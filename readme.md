...
引入（附上projecy syllabus和lecture notes）:

```bash
我是香港科技大学一位辅修心里学的學生，我正在上一門課叫sosc2210 social psychology，裡面有一個group project要做。group project的要求文件在這裡，你要嚴格參考這個文件的內容。同時，我也會附上一些lecture notes讓你知道我們現在在課上學過些什麼內容(group project topic不一定要从學過的東西裡面中推導，可以是任意topic)，以及我們學校心理學的講義和教學風格是怎麼樣。现在，我希望你可以从我们的角度出发，为我们的project提一个大体建议，包括但不限于：为我们提供大量的project related的相关资料以拓展相关的视野和学习，提出潜在topic，提供大量的相关文献给我们参考
```

一些常见的topic发掘和experiment验证prompt:

```bash
現在，我需要你从我們group project組員的角度出發，為我想10個貼切實際(可執行的)，有意義，內容豐富，可以為我們group project獲取 A+的topic，並且對每個topic附上explaination和介紹。
```

```bash
现在，我需要你站在我们心理学学生做group project的角度，用英文为我写一篇关于我们group project的详细proposal，用于下周向instructor介绍我们的group project。内容要是详细的，内容要参照我们上面所设计的实验和情况。你也可以加入你认为对我们有用的拓展内容进去proposal里面。现在，开始写proposal
```

```bash
现在这里是我们目前project和experiemnt的设置。现在我要问你一些对于我们experiment的想法，我需要你以绝对客观，不阿谀奉承的角度来回答我。1.对我们的topic 和experemnt整体有什么想法吗? 2. expeeriemnt 整体可行吗？ 3.有没有什么我们设计漏了的东西或者你发现的重大flaws？,4.你对我们的expeiemnt有什么大量的补充或者大量的补充知识可以告诉我吗？
```

```bash
现在我更新了一下我的实验设计和加了一些细节，你了解一下我的最新设计：
```
